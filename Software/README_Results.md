...
`Results.ipynb` is the post-processing and validation notebook for the Bubble Evaluation Tool. It is intended to be run after the main tracking notebook or GUI has generated bubble tracking outputs. The notebook reads tracking CSV files, averages/settings files, optional ground-truth files, and optional output videos, then generates figures, summary tables, validation metrics, uncertainty estimates, and batch-level comparison outputs.
...
RunName\_averages.csv

RunName\_gt.csv

RunName\_pred\_mask.mp4

RunName\_filtered.mp4

RunName\_gt\_mask.mp4
...
Only `RunName\_tracked.csv` is strictly required. However, additional files enable more complete analysis.
...
| Track ID | `id`, `ID`, `track\_id` |

| Centroid x | `cx`, `x`, `centroid\_x` |

| Centroid y | `cy`, `y`, `centroid\_y` |

| Diameter | `diam\_px`, `diam`, `diameter\_px` |

| Speed | `vel\_px\_s`, `speed\_px\_s`, `vel`, `speed` |

| x-velocity | `vel\_x\_px\_s`, `vx\_px\_s`, `vx` |

| y-velocity | `vel\_y\_px\_s`, `vy\_px\_s`, `vy` |
...
| GT centroid x | `gt\_cx\_px`, `gt\_cx`, `cx`, `x` |

| GT centroid y | `gt\_cy\_px`, `gt\_cy`, `cy`, `y` |

| GT diameter | `gt\_diam\_equiv\_px`, `gt\_diam\_eq`, `gt\_diam\_px`, `diam\_px`, `diam` |
...
| GT speed | `gt\_speed\_px\_s`, `gt\_speed`, `speed\_px\_s`, `speed` |

| GT x-velocity | `gt\_vx\_px\_s`, `gt\_vx`, `vx` |

| GT y-velocity | `gt\_vy\_px\_s`, `gt\_vy`, `vy` |
...
This video contains the final filtered/annotated tracking output.
...
It is used with `RunName\_pred\_mask.mp4` to compute semantic segmentation IoU and create mask-difference panels.
...
The notebook enters non-GT mode when a matching `RunName\_gt.csv` file is not found.
...
The notebook enters GT mode when a matching `RunName\_gt.csv` file is found.
...
&#x20;       fig\_batch\_overlay\_diameter.png

&#x20;       fig\_batch\_overlay\_velocity.png

&#x20;       fig\_grace\_all\_runs\_avg.png



&#x20;       accuracy/

&#x20;           fig\_batch\_diam\_MAE.png

&#x20;           fig\_batch\_diam\_RMSE.png

&#x20;           fig\_batch\_diam\_Bias.png

&#x20;           fig\_batch\_speed\_MAE.png

&#x20;           fig\_batch\_speed\_RMSE.png

&#x20;           fig\_batch\_speed\_Bias.png
...
\## `per\_track\_mean\_diam\_vel.csv`
...
\## `bubble\_density\_per\_frame.csv`
...
\## `centroid\_uncertainty\_summary.csv`
...
\## `centroid\_uncertainty\_per\_track.csv`
...
\## `diameter\_uncertainty\_summary.csv`
...
\## `diameter\_uncertainty\_per\_track\_px.csv`
...
diameter\_uncertainty\_per\_track\_in.csv
...
\## `standardize\_values\_speed.csv`
...
fig\_trajectories\_2d\_timecolored.png
...
fig\_velocity\_vs\_diameter\_\*.png
...
fig\_mean\_vel\_vs\_mean\_diam\_per\_track\_\*.png
...
fig\_velocity\_vs\_time\_smoothed\_\*.png
...
fig\_velocity\_smoothness\_\*.png
...
fig\_qualitative\_sequence.png
...
fig\_measured\_vs\_true\_diameter\_\*.png
...
fig\_measured\_vs\_true\_speed\_\*.png
...
fig\_error\_hist\_diameter\_\*.png
...
fig\_relative\_error\_diameter.png
...
fig\_error\_vs\_time\_diameter\_\*.png
...
fig\_error\_vs\_time\_speed\_\*.png
...
batch\_overlays/fig\_batch\_overlay\_diameter.png
...
batch\_overlays/fig\_batch\_overlay\_velocity.png
...
batch\_overlays/fig\_grace\_all\_runs\_avg.png
...
These plots are generated only when GT cases are present in `master\_summary.csv`.
...
To get inches and inches per second, check that `RunName\_averages.csv` contains valid scale information.
...
2\. Confirm that `RunName\_tracked.csv` and `RunName\_averages.csv` were generated.
...
8\. Review `run\_summary.csv`, main figures, and `errors.log`.
...
1\. Confirm that `RunName\_tracked.csv` exists.

2\. Confirm that `RunName\_gt.csv` exists.

3\. If doing IoU validation, confirm that `RunName\_gt\_mask.mp4` and `RunName\_pred\_mask.mp4` exist.
...
9\. Review `matched\_pairs.csv`, GT validation plots, IoU outputs, and tracking metrics.
